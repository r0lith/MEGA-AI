...
module.exports = async function weatherCommand(sock, chatId, message, city) {
    try {
        if (!city || city.trim() === '') {
            await sock.sendMessage(chatId, { text: "❌ Please provide a place to search.\nExample: .weather Karachi" }, { quoted: message });
            return;
        }
        ...
        await sock.sendMessage(chatId, { text: '❌ Sorry, I could not fetch the weather. Make sure the place name is correct.' }, { quoted: message });
    }
};
